...
def detect_needle(frame):
    ...
    # Restrict to gauge face circle only
    gauge_mask = np.zeros_like(dark_mask)
    cv2.circle(gauge_mask, (PIVOT_X, PIVOT_Y), 220, 255, -1)
    needle_region = cv2.bitwise_and(dark_mask, gauge_mask)

    # Exclude the red frame border — erode slightly inward
    erode_kernel = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (5, 5))
    gauge_mask_eroded = cv2.erode(gauge_mask, erode_kernel, iterations=3)
    needle_region = cv2.bitwise_and(needle_region, gauge_mask_eroded)
